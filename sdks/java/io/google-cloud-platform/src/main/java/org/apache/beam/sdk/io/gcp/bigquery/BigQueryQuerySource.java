...
class BigQueryQuerySource<T> extends BigQuerySourceBase<T> {
  ...
  private BigQueryQuerySource(
    ...
    this.queryHelper = new BigQueryQueryHelper(query, flattenResults, useLegacySql);
  }

  @Override
  public long getEstimatedSizeBytes(PipelineOptions options) throws Exception {
    BigQueryOptions bqOptions = options.as(BigQueryOptions.class);
    return queryHelper.dryRunQueryIfNeeded(bqServices, bqOptions).getTotalBytesProcessed();
  }

  @Override
  protected TableReference getTableToExtract(BigQueryOptions bqOptions)
      throws IOException, InterruptedException {
    return queryHelper.executeQuery(bqServices, bqOptions, stepUuid);
  }
